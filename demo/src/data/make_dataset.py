...
first_part = Path(f).stem.split("_")[0]
...
data_merged.iloc[:100].resample("200ms").agg(sampling)
 
# split by day 
days = [g for n, g in data_merged.groupby(pd.Grouper(freq="D"))]

data_resampled = pd.concat(
    [df.resample("200ms").agg(sampling).dropna() for df in days]
)
...
# print("acc labels:", sorted(acc_df["label"].dropna().unique()))
# print("gyr labels:", sorted(gyr_df["label"].dropna().unique()))
# print("merged labels:", sorted(data_merged["label"].dropna().unique()))
# print("resampled labels:", sorted(data_resampled["label"].dropna().unique()))
